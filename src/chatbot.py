...
def start():
    ...
    logging.info("workflow stage: %s", streamlit.session_state.stage.name)
    ...
    if not mistral_api_key:
        mistral_api_key = streamlit.text_input(
            "Mistral API Key", type="password")
    if not mistral_api_key:
        streamlit.info(
            _("Please add your Mistral API key to continue."), icon="🗝️")
    else:
        ...
        if streamlit.session_state.stage != WorkflowStage.ANALYSIS:
            ...
            with user_input.container():
                user_description = streamlit.text_area(
                    _("Case description..."), height=150, value=streamlit.session_state.user_description)
        ...
        if streamlit.session_state.stage == WorkflowStage.INITIAL:
            ...
            with col2:
                move_forward_button = streamlit.button(
                    _("Move Forward"), type="primary")
